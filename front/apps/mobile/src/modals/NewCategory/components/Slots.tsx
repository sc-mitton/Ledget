...
const Slots = (props: AnimatedNumbersProps) => {
  ...
  useLayoutEffect(() => {
    ...
    if (props.includeComma) {
      const newCommaPositions = getNewCommaPositions(
        stringValue, commaPositions, parseFromLeft, props.precision);
  ...
  const onCompleted = useCallback(() => {
    const cleanedSlots = slots
      ...
      .map(s => Number.isFinite(s[0][1]) ? [[s[0][1]], s[1]] as SlotValue : [[s[0][0]], s[1]] as SlotValue)
        ...
        {slots.map((slot, i) => {
          ...
          return (
            <Fragment key={`${idRef.current}-${slot[1]}`}>
              ...
              />
              {(props.precision || 0) > 0 && props.precision === (slots.length - 1 - i) &&
