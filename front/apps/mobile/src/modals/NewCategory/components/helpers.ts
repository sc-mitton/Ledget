...
export const getNewSlotValues = (newValue: string, currentSlots: SlotValue[], parseFromLeft: boolean) => {
  ...
  const newSlotValues = Array.from({ length: iterLength }, (_, i) => [i] as SlotValue[0]);
...
export const getNewCommaPositions = (newValue: string, currentCommaPositions: CommaPosition[], parseFromLeft: boolean, precision?: number) => {
  ...
  return newCommaPositions.concat(
    Array.from({ length: precision || 0 }, () => null)
