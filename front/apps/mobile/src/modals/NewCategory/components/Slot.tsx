...
const Slot = (props: SlotProps) => {
  ...
  const [valA, setValA] = useState<number | undefined>(props.slot[0][0] as number)
  ...
  useLayoutEffect(() => {
    ...
    // Adding slot
      ...
      width.value = props.spring
        ...
        : withTiming(
          ...
          { duration: props.animationDuration / 2, easing: reEasing })
    ...
    // Animating slot
      ...
      Animated.timing(
        shown === 'A' ? yB : yA,
        { toValue: finalY, ...timedConfig, duration: 0 }
      ).start(() => {
  ...
  useEffect(() => {
    if (typeof valA === 'number' && typeof valB === 'number') {
      ...
      const sequence = [
        Animated.timing(
          shown === 'A' ? yB : yA,
          { toValue: shownValFinalY * -1, duration: 0, useNativeDriver: true }
        ),
        props.spring
