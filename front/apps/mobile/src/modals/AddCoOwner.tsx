  ...
  return (
    <View key="2">
      <Text color="secondaryText" style={styles.text}>
        To finish, have your new account member scan the qr code or follow the
        link
      </Text>
  ...
  return (
    <View key="1">
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <Fragment>
          <Text color="secondaryText" style={styles.text}>
            Enter the email address of the person you'd like to add to your
            account
          </Text>
          <Controller
            control={control}
          ...
          <Seperator backgroundColor="modalSeperator" />
          <Text variant="footer" style={styles.footer}>
            Members on your account will be able to see all of your data
            including account information, transactions, bills, and budget
            categories. Only add members you trust.
          </Text>
        </Fragment>
...
const AddCoOwner = (props: ModalScreenProps<'AddCoOwner'>) => {
  ...
  return (
    <Modal>
      ...
      {isSuccess ? (
        <SlideView position={1} skipExit={true}>
          <Slide2 />
        </SlideView>
      ) : (
        <SlideView position={0} skipEnter={true} skipExit={!isSuccess}>
          <Slide1 />
        </SlideView>
      )}
    </Modal>
