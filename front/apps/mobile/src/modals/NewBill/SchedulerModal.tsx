...
const DAYS = Array.from({ length: 31 }, (_, i) => i).reduce(
  (acc, day) => {
...
const SchedulerModal = (props: ModalProps) => {
  const {
    field: { onChange: onPeriodChange, value: periodValue },
  } = useController({ control: props.control, name: 'period' });
  const {
    field: { onChange: onScheduleChange, value: scheduleValue },
  } = useController({ control: props.control, name: 'schedule' });
  const {
    field: { onChange: onExpiresChange, value: expiresValue },
  } = useController({ control: props.control, name: 'expires' });
      ...
      >
        <TextInputbase error={props.error}>
          <View style={styles.calendarIcon}>
            <Icon
              icon={Calendar}
              color={scheduleValue ? 'mainText' : 'placeholderText'}
            />
          </View>
          <Text color={scheduleValue ? 'mainText' : 'placeholderText'}>
            {scheduleValue
              ? getScheduleDescription(scheduleValue)
              : 'Set schedule'}
          </Text>
        </TextInputbase>
      </TouchableOpacity>
            ...
            <View style={styles.inputs}>
                ...
                {periodValue === 'month' && tab === 1 && (
                  <>
                          ...
                          (week) => (
                            <Button
                              ...
                              onPress={() => {
                                onScheduleChange({
                                  week: week,
                                  week_day: scheduleValue?.week_day,
                                });
                              }}
                            />
                          )
                    ...
                    <Text variant="footer" fontSize={15}>
                      Bill will repeat every month on the selected week and day
                    </Text>
                  </>
                )}
              </Box>
              ...
              {periodValue !== 'once' && (
                <DatePicker
                  ...
                  disabled={[
                    dayjs()
                      .month(scheduleValue?.month || dayjs().month() - 1)
                      .date(scheduleValue?.day || dayjs().date()),
                    undefined,
                  ]}
                  defaultValue={expiresValue ? dayjs(expiresValue) : undefined}
                  onChange={(value) =>
                    value
                      ? onExpiresChange(value?.toISOString())
                      : onExpiresChange(undefined)
                  }
                  icon
                />
              )}
