          ...
          {props.route.params?.state === 'picking' && (
            <Animated.View
              ...
              style={[
                ...
                { backgroundColor: theme.colors.widgetPickerBackground },
              ]}
            />
          )}
          <Box paddingTop="statusBar" style={[styles.header]}>
            ...
            <View style={styles.headerRight}>
              {['editing', 'picking'].includes(props.route.params?.state) && (
                <Button
                  label={
                    props.route.params?.state === 'editing' ? 'Done' : 'Close'
                  }
                  variant={'bold'}
                  borderRadius="circle"
                  marginTop="xs"
              ...
              {['dropping', 'idle'].includes(props.route.params?.state) && (
                <>
                  {selectedStoredWidgets.length > 0 && (
                    <Button
                      onPress={() => {
                        props.navigation.setParams({ state: 'editing' });
                      }}
                      variant="square"
            ...
            <View style={styles.graphic}>
              ...
              <View style={styles.message}>
                <Text color="tertiaryText" textAlign="center">
                  Welcome to your custom dashboard where you can add widgets to
