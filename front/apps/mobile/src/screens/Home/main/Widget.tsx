...
const Widget = (props: WidgetProps) => {
  ...
  const scale = useSharedValue(props.widget.id ? 1 : 0.5);
  ...
  useAnimatedReaction(
    ...
    (height) => {
      width.value =
        props.widget.shape === 'rectangle' && props.widget.id
          ? height.value * 2 + gap
          : height.value;
  ...
  useAnimatedReaction(
    ...
    (newPositions) => {
      if (
        ...
        // Update column
        column.value =
          newPositions.value[props.widget.id || props.widget.type][0] % 2;
  ...
  useEffect(() => {
    if (props.state === 'editing') {
      ...
      overlayOpacity.value = withTiming(0.95, { duration: 200 });
    } else {
  ...
  useEffect(() => {
    if (
      isDragging.value ||
      props.widget.id ||
      !props.positions.value[props.widget.type]
    )
      return;
    ...
    if (props.state === 'picking') {
      ...
      scale.value = withDelay(500, withSpring(0.5, defaultSpringConfig));
  ...
  const dispatchWidgetUpdates = useCallback(() => {
    const currentIndex = props.order.value.findIndex(
      (k) => k === props.widget.id || k === props.widget.type
    );
    const newIndex =
      column.value === 1 && props.widget.shape === 'square'
        ? currentIndex - 1
        : currentIndex;
    ...
    dispatch(
      updateWidget({
        widget: {
          ...props.widget,
          shape: props.widget.shape === 'rectangle' ? 'square' : 'rectangle',
        },
        index: newIndex,
      })
    );
  ...
  const pan = Gesture.Pan()
    ...
    .failOffsetY(50)
    .failOffsetX([
      (-1 * props.height.value) / 2,
      props.height.value * 2.5 + gap,
    ])
    .onStart((ctx) => {
      ...
      shadowOpacity.value =
        theme.colors.mode === 'dark' ? withTiming(0.3) : withTiming(0.1);
    ...
    .onChange(({ translationX: tx, translationY: ty, changeX, changeY }) => {
      ...
      if (
        ...
        const iterator = Array.from(
          { length: props.order.value.length },
          (_, i) =>
        ...
        for (const i of iterator) {
          ...
          const initiateReorder =
            direction > 0
              ? props.widget.shape === 'rectangle'
                ? nextGridPos &&
                  nextGridPos[0] + nextGridPos[1] - 1 >= newGridPos[0] + 1
                : nextGridPos && nextGridPos[0] >= newGridPos[0]
              : props.widget.shape === 'rectangle'
              ? nextGridPos &&
                nextGridPos[0] - nextGridPos[1] + 1 <= newGridPos[0]
              : nextGridPos && nextGridPos[0] <= newGridPos[0];
          ...
          if (initiateReorder) {
            ...
            const hasSquareNeighbor =
              currentIndex === props.order.value.length - 1
                ? false
                : column.value === 0
                ? props.positions.value[
                    props.order.value[currentIndex + 1]
                  ]?.[1] === 1
                : props.positions.value[
                    props.order.value[currentIndex - 1]
                  ]?.[1] === 1;
            ...
            if (currentIndex > -1) {
              ...
              if (
                ...
                const startSnip =
                  column.value === 0 ? currentIndex : currentIndex - 1;
      ...
      const upperBound =
        lowerBound +
        props.containerHeight.value -
        theme.spacing.navHeight -
        props.height.value * 0.75;
    ...
    .onEnd(({ translationX, translationY }) => {
      ...
      const finalGridPosition = Math.min(
        gridPosition[0],
        props.positions.value[
          props.order.value[props.order.value.length - 1]
        ]?.[0] || 0
      );
      ...
      finalPosition.y = props.widget.id
        ? finalPosition.y
        : finalPosition.y + props.scrollY.value;
    ...
    .onChange(({ changeX }) => {
      ...
      if (props.widget.shape === 'square') {
        ...
        const needsUpdating =
          column.value === 0
            ? (props.positions.value[
                ...
                (props.positions.value[props.widget.id!]?.[0] || 0) ==
              1
            : props.positions.value[props.order.value[currentIndex - 1]] <
        ...
        if (column.value === 0) {
          ...
          updatedGridPositions[props.widget.id || props.widget.type] =
      ...
      if (Math.abs(tx) > props.height.value / 2) {
        ...
        if (props.widget.shape === 'square') {
          ...
          const returnedPositionsMap = Object.fromEntries(
            returnedGridPositions.map((pos, index) => [
              storedWidgets[index].id,
              pos,
            ])
          );
  ...
  return (
    <Animated.View style={style}>
      <Box
        ...
        shadowOpacity={theme.colors.mode === 'dark' ? 0 : 0.2}
        shadowRadius={12}
