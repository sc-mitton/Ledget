...
import { selectBudgetMonthYear, selectCategoryMetaData } from '@ledget/shared-features'
import { useAppearance } from '@/features/appearanceSlice';

const SquareFilled = (props: WidgetProps & { loading: boolean }) => {
  const { month, year } = useAppSelector(selectBudgetMonthYear);
  const { monthly_spent, yearly_spent } = useAppSelector(selectCategoryMetaData);
  ...
  return (
    <Box padding='xxs' style={sharedStyles.box}>
      ...
      <View style={sharedStyles.totalAmount}>
        <DollarCents value={Big(monthly_spent).plus(yearly_spent).toNumber()} withCents={false} variant='bold' fontSize={28} />
      </View>
      <Box style={sharedStyles.bottomRow} paddingHorizontal='xxxs'>
        ...
        <View style={sharedStyles.bottomRowCell}>
          <TouchableOpacity
            ...
            onPress={() => Alert.alert(
              'Yearly Spending',
              `Resets every year on ${dayjs(user?.yearly_anchor).format('MMM D')}`,
              [{ text: 'OK' }],
          ...
          <DollarCents value={Big(yearly_spent).toNumber()} withCents={false} fontSize={15} />
        </View>
      </Box>
    </Box>
  )
...
const RectangleFilled = (props: WidgetProps & { loading: boolean }) => {
  const { month, year } = useAppSelector(selectBudgetMonthYear);
  const { monthly_spent, yearly_spent, limit_amount_monthly, limit_amount_yearly } = useAppSelector(selectCategoryMetaData);
  const { data: user } = useGetMeQuery()
  const { mode } = useAppearance()
    ...
    <Box paddingRight='s' paddingLeft='l' style={styles.rectangleBox}>
      <View style={styles.rectangleLeft}>
        <View>
          ...
          <View style={styles.totalAmount}>
            {props.loading
              ? <Box marginTop='s' ><PulseBox width={100} height='l' borderRadius='s' /></Box>
              : <DollarCents value={Big(monthly_spent).plus(yearly_spent).toNumber()} withCents={false} variant='bold' fontSize={24} />}
          </View>
        </View>
      </View>
      <Box
      ...
      >
        <View style={styles.rectangleRightColumn}>
          <View>
            <TouchableOpacity activeOpacity={.7} onPress={() => Alert.alert(
              'Monthly Spending',
              `You've spent $${Big(monthly_spent).toNumber()} this month from your monthly spending categories`,
              [{ text: 'OK' }],
              { userInterfaceStyle: mode }
            )}>
              <View style={sharedStyles.bottomTitle}>
            ...
            {props.loading
              ? <Box marginTop='xs' ><PulseBox width={54} height='reg' borderRadius='s' /></Box>
              : <DollarCents value={Big(monthly_spent).toNumber()} withCents={false} fontSize={15} />}
          </View>
          <View>
            <TouchableOpacity activeOpacity={.7} onPress={() => Alert.alert(
          ...
          <View style={styles.cell}>
            ...
            <View>
              <TouchableOpacity activeOpacity={.7} onPress={() => Alert.alert(
                ...
                'You have ' + formatter.format(Big(limit_amount_monthly).minus(monthly_spent).div(100).toNumber()) + ' left from your monthly spending categories',
                [{ text: 'OK' }],
                { userInterfaceStyle: mode }
              )}>
                <View style={sharedStyles.bottomTitle}>
              ...
              {props.loading
                ? <Box marginTop='xs' ><PulseBox width={54} height='reg' borderRadius='s' /></Box>
                : <DollarCents value={Big(limit_amount_monthly).minus(monthly_spent).toNumber()} withCents={false} fontSize={15} />}
            </View>
          </View>
          <View style={styles.cell}>
            <Box backgroundColor='menuSeperator' style={styles.divider} />
            <View>
              <TouchableOpacity activeOpacity={.7} onPress={() => Alert.alert(
                ...
                'You have ' + formatter.format(Big(limit_amount_yearly).minus(yearly_spent).div(100).toNumber()) + ' left from your yearly spending categories',
                [{ text: 'OK' }],
                { userInterfaceStyle: mode }
              )}>
                <View style={sharedStyles.bottomTitle}>
              ...
              {props.loading
                ? <Box marginTop='xs' ><PulseBox width={54} height='reg' borderRadius='s' /></Box>
                : <DollarCents value={Big(limit_amount_yearly).minus(yearly_spent).toNumber()} withCents={false} fontSize={15} />}
            </View>
          </View>
        </View>
      </Box>
