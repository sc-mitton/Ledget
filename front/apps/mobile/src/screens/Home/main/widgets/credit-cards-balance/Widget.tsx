...
const SELECT_OPTION_HEIGHT = SELECT_OPTION_WIDTH * 0.66;
const MAX = 3;
...
const Selector = (widget: WidgetProps<{ accounts: string[] }>) => {
  ...
  useEffect(() => {
    if (accountsData && !widget.args) {
      ...
      setAccounts(
        accountsData.accounts.filter((a) =>
          widget.args?.accounts?.includes(a.id)
        )
      );
    }
      ...
      {accounts || accountOptions ? (
        <Carousel
          ...
          renderItem={({ item: account, index }) => {
            const primary_color = accountsData?.institutions.find(
          ...
          onProgressChange={(p) => {
            ...
            const carouselLength = accounts
              ? accounts.length
              : accountOptions?.length || 0;

            if (p - progress.value > 20) {
              setCarouselIndex(
                carouselIndex - 1 >= 0 ? carouselIndex - 1 : carouselLength - 1
              );
              carouselIndexLock.current = true;
              progress.value = p;
            } else if (p - progress.value < -20) {
              setCarouselIndex(
                carouselIndex + 1 < carouselLength ? carouselIndex + 1 : 0
              );
              carouselIndexLock.current = true;
              progress.value = p;
            }
        ...
        <View style={styles.skeletonContainer}>
          {Array.from({ length: 3 }).map((_, index) => (
            <View
              style={{
                width: SELECT_OPTION_WIDTH * 0.95,
                height: SELECT_OPTION_HEIGHT * 0.95,
              }}
            >
              <Box
                style={[
          ...
          {accounts ? (
            <View style={styles.bottomRow}>
              <SlotNumbers
                ...
                value={Big(
                  accounts[carouselIndex].balances?.current || 0
                ).toNumber()}
