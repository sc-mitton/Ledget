...
export const BakedCarousel = (props: CarouselProps) => {
  ...
  const maxIncome = useMemo(() =>
    carouselData?.reduce((max, breakdown) => Math.max(max, breakdown.income), 0) || 0
    , [carouselData])
      ...
      {carouselData &&
        <Carousel
          ...
          renderItem={({ item: breakdown, index }) => {
            const investmentPercent = (breakdown.investment_transfer_out / (breakdown.income || 1)) * 100
            ...
            return (
              <View style={[styles.carouselItem, { opacity }]}>
                ...
                />
                <Box
                  style={[styles.bar, { zIndex: investmentPercent > savedPercent ? 0 : 1, height: `${savedPercent ? 100 : 0}%` },]}
                  backgroundColor='greenText'
                />
                <Box style={[styles.bar, { height: `${incomePercent}%` }]} backgroundColor='transactionShimmer' />
              </View>
            )
          }}
