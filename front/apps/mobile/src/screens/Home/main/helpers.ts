...
export const getPositionsMap = (widgets: Widget[]) => {
  const positionEntries = getGridPositions(widgets).map(
    (gridPosition, index) => [
      widgets[index].id || widgets[index].type,
      gridPosition,
    ]
  );
...
export const getGridPositions = (widgets: Widget[]) => {
  ...
  const gridPositions = widgets.reduce((acc, w) => {
    ...
    const gridPosition =
      acc.last?.shape === 'square' || !acc.last
        ? w.shape === 'square'
          ? lastGridPosition + 1
...
export const getNewGridPosition = (
  ...
  const col =
    proximityX < PROXIMITY_THRESHOLD ? Math.round(x) - 1 : gridPosition % 2;
