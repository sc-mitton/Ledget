...
const AddBills = (props: OnboardingScreenProps<'AddBills'>) => {
  const { data: recurringTransactions, isLoading: isLoadingRecurringTransactions } = useGetRecurringTransactionsQuery();
  ...
  const { field: { onChange: onLowerAmountChange, value: lowerAmountValue } } = useController({ control, name: 'lower_amount' });
  const { field: { onChange: onUpperAmountChange, value: upperAmountValue } } = useController({ control, name: 'upper_amount' });
  ...
  const saveCustomBill = () => {
    handleSubmit((data) => {
      ...
      const lowerAmount = lower_amount === undefined ? undefined : Big(lower_amount).div(100).toNumber()
      if (flatListData.some(b => b.id === modalBillId)) {
        setFlatListData(prev => prev.map(b => b.id === modalBillId ? { ...restData, id: b.id, lower_amount: lowerAmount, upper_amount: upperAmount } as any : b))
      } else {
        setFlatListData([...flatListData, { ...restData, upper_amount: upperAmount, lower_amount: lowerAmount } as any])
      }

      if (selectedBills.some(b => b.id === modalBillId)) {
        setSelectedBills(prev => prev.map(b => b.id === modalBillId ? { ...data, id: b.id } as any : b))
      } else {
  ...
  useEffect(() => {
    if (recurringTransactions) {
      setFlatListData(
        recurringTransactions.map(t => ({ ...t, id: Math.random().toString(36).slice(2, 9) }) as any)
      )
    }
  ...
  useEffect(() => { setTimeout(() => { setRowsMeasured(true) }, 1000) }, []);

  return (
    <Box variant='screen'>
      <View style={sharedStyles.mainContainer}>
        <View style={sharedStyles.header}>
          <Text fontSize={24} lineHeight={28} variant='geistSemiBold' marginBottom='s'>
            Add Bills
          </Text>
        ...
        <Box variant='nestedContainer' style={styles.form}>
          ...
          <View>
            <Canvas style={[sharedStyles.topMask, sharedStyles.mask]}>
              <Rect x={0} y={0} width={Dimensions.get('window').width} height={24}>
                <LinearGradient
                  colors={[
                ...
                return (
                  <View style={[styles.suggestionOptionContainer]}>
                    <Swipeable
                      ...
                      friction={2}
                      containerStyle={[{ backgroundColor: theme.colors.modalSeperator }, styles.swipeable]}
                        ...
                        <Box style={styles.rightActions}>
                          <Button
                            onPress={() => {
                              ...
                              if (isBill(item) && item.lower_amount !== undefined && item.upper_amount !== undefined) {
                                setValue('range', true)
                              }
                              if ('emoji' in item) {
                              ...
                              if (item.lower_amount) {
                                setValue('lower_amount', Big(item.lower_amount).times(100).toNumber())
                              }
                              setValue('upper_amount', Big(item.upper_amount).times(100).toNumber())
                              setValue('range', isBill(item) ? 'lower_amount' in item : false)
                              for (const period of ['day', 'month', 'week', 'week_day', 'year'] as const) {
                                setValue(`schedule.${period}`, item[period])
                              }
                              setValue('reminders', isBill(item) ? item.reminders : [])
                              setModalBillid(item.id)
                              setShowModal(true)
                            }}
                            icon={<Icon icon={Edit2} size={18} color='secondaryText' />}
                          />
                        </Box>}
                    >
                      <Box
                      ...
                      >
                        ...
                        <Button
                          ...
                          <View style={styles.suggestionOptionContent}>
                            ...
                            <DollarCents value={Big(item.upper_amount).times(100).toNumber()} />
                          </View>
                        </Button>
                      </Box>
                    </Swipeable>
          ...
          <View style={styles.bottomButtons}>
            ...
            <Text color='quinaryText' marginTop='s'>
              <MaterialCommunityIcons name="gesture-swipe-left" size={24} color={theme.colors.quinaryText} />
              &nbsp;Edit
            </Text>
          </View>
        </Box>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Box style={styles.moneyInputs}>
                <MoneyInput
                  ...
                  defaultValue={
                    isRange
                      ? lowerAmountValue !== undefined && upperAmountValue !== undefined
                        ? [lowerAmountValue, upperAmountValue] as [number, number]
