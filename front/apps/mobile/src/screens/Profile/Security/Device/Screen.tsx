...
const Device = ({ navigation, route }: SecurityScreenProps<'Device'>) => {
  ...
  const [
    removeDevice,
    { isLoading: processingDelete, isSuccess: successfulDelete },
  ] = useRemoveRememberedDeviceMutation();
  ...
  useEffect(() => {
    if (groupedDevices) {
      setDevice(
        groupedDevices.find(
          (device) => device[0] === route.params.key.join(',')
        )
      );
    }
  }, [groupedDevices]);
      ...
      <Box variant="screen" style={styles.container}>
        <View style={styles.content}>
          <View style={styles.header}>
            <Text variant="header2">{device?.[0].split(',')[0]}&nbsp;</Text>
            <View
              style={
                iconKey === 'is_mobile' ? styles.mobileIcon : styles.pcIcon
              }
            >
              <Icon
                color="secondaryText"
            ...
            <View
              style={
                iconKey === 'is_pc' ? styles.pcLocation : styles.mobileLocation
              }
            >
                    ...
                    <View style={styles.session}>
                      <View style={styles.row}>
                        ...
                        <Text style={styles.lastLogin}>
                          <Text
                            color="tertiaryText"
                            style={styles.lastLoginHeader}
                          >
                            Last Login{' '}
                          </Text>
                          {dayjs(session.last_login).format(
                            'MMM D, YYYY h:mm A'
                          )}
                        </Text>
                        {session.current_device && (
                          <Text color="greenText" marginTop="xs">
                            Current Session
                          </Text>
                        )}
                      </View>
                      {!session.current_device && (
                        <SubmitButton
                          ...
                          backgroundColor={
                            processingDelete && removedDevice === session.id
                              ? 'transparent'
                              : 'lightGrayButton'
                          }
                          borderRadius={'m'}
                          ...
                          isSuccess={
                            successfulDelete && removedDevice === session.id
                          }
                        >
