...
const Alerts = ({ category }: { category: Category }) => {
  const [updateCategories, { isLoading, isSuccess }] =
  ...
  return (
    <>
            ...
            .map((alert, index) => (
              <Fragment key={`alert-${index}`}>
                ...
                <View style={styles.alert}>
                  ...
                  <DollarCents
                    value={Big(category.limit_amount)
                      .times(alert.percent_amount)
                      .div(100)
                      .toNumber()}
                  />
      ...
      <Text variant="footer" paddingHorizontal={'s'}>
        Get notified when you've reached these spending amounts for this
        category
      </Text>
    </>
  );
