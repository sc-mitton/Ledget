...
export default function DatePicker() {
  ...
  useEffect(() => {
    ...
    const startYear = dayjs(user?.created_on).year();
    ...
    for (let y = startYear, i = 0; y <= dayjs().year(); y++, i++) {
      const startMonth =
        y === dayjs(user?.created_on).year()
          ? dayjs(user?.created_on).month() + 1
          : 1;
        ...
        >
          <Box paddingTop="statusBar">
            ...
            <View style={styles.pagerContainer}>
              <Button
                onPress={() => {
                  pagerRef.current?.setPage(Math.max(0, pagerIndex - 1));
                }}
                padding="none"
              ...
              {options && (
                <PagerView
                  ref={pagerRef}
                  initialPage={year - dayjs(user?.created_on).year()}
