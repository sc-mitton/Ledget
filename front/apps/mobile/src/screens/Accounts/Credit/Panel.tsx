...
export default function Panel(props: AccountsTabsScreenProps<'Credit'>) {
    ...
    if (accountsData) {
      setAccounts(
        accountsData.accounts.filter(
          (account) => account.type === props.route.name.toLowerCase()
        )
      );
    }
  }, [accountsData]);
  ...
  useEffect(() => {
    if (props.route.params?.account && accounts) {
      carouselRef.current?.scrollTo({
        count:
          accounts?.findIndex(
            (account) => account.id === props.route.params.account?.id
          ) || 0,
        animated: true,
      });
    }
  ...
  const [cardTransparencies, cardsApi] = useSprings(
    accounts?.length || 0,
    (i) => ({
      from: { opacity: 1 },
              ...
              <Carousel
                ...
                renderItem={({ item, index }) => (
                  ...
                  >
                    <CarouselItem
                      account={item}
                      onPress={() =>
                        props.navigation.navigate('AccountsTabs', {
                          screen: 'Credit',
                          params: { account: item },
                        })
                      }
                ...
                onProgressChange={(p) => {
                  ...
                  if (p - progress.value > 20) {
                    ...
                    setCarouselIndex(
                      carouselIndex + 1 < accounts.length
                        ? carouselIndex + 1
                        : 0
                    );
                    carouselIndexLock.current = true;
