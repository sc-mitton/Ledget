...
import { ArrowUpRight, ArrowDownRight, ArrowRight, ChevronRight } from 'geist-native-icons'
...
import { useGetInvestmentsQuery, isInvestmentSupported, Holding, selectTrackedHoldings } from '@ledget/shared-features';
          ...
          >
            {holdings?.map((holding, index) => {
              ...
              if (holding.security_id && trackedHoldings[holding.security_id].length > 1) {
                ...
                percent_change = Big(current_institution_value).minus(previous_institution_value).div(previous_institution_value).times(100).toNumber()
              }
                  ...
                  <Box style={styles.holding} >
                    <View style={styles.holdingTitle}>
                      ...
                      <View style={styles.holdingTrend}>
                        <Text
                          fontSize={14}
                          color={percent_change !== undefined ? (percent_change < 0 ? 'redText' : 'greenText') : 'tertiaryText'}>
                          {percent_change !== undefined ? `${percent_change}%` : '—'}
                        </Text>
                        {percent_change !== undefined
                          ?
                          <Icon
                            icon={percent_change === 0 ? ArrowRight : percent_change < 0 ? ArrowDownRight : ArrowUpRight}
                            size={13}
