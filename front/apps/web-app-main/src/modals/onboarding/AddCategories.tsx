...
const CategoriesColumn = ({ period }: { period: Period }) => {
  ...
  const [editAmountIndex, setEditAmountIndex] = useState<number | boolean>(
    false
  );
  const [editValue, setEditValue] = useState<number>();

  const { items, transitions, containerProps, api } =
...
const SuggestedTabPanel = () => {
  ...
  return (
    <Tab.Panel className={styles.suggestedItemsContainer} as="div">
      {periodTabIndex === 0
        ? monthRecommendations
            .filter((r) => !monthItems.find((i) => i.name === r.name))
            .map((item, index) => {
              return (
                <BillCatLabel
                  as="button"
        ...
        : yearRecommendations
            .filter((r) => !yearItems.find((i) => i.name === r.name))
            .map((item, index) => {
              return (
                <BillCatLabel
                  as="button"
