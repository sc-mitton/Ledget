...
const NoteInnerWindow = ({ item }: { item: Transaction }) => {
    ...
    (e: any) => {
      if (e.key === 'Escape') {
        ...
        notesContainerRef.current
          ?.querySelector<HTMLTextAreaElement>(`[data-focused='true']`)
          ?.blur();
      } else if (e.key === 'Enter') {
        e.stopPropagation();
        if (a11yNote === undefined) {
          ...
          setFocusedNoteId(
            notes
              ...
              .find((_, index) => index === a11yNote)?.id
          );
        }
        notesContainerRef.current
          ?.querySelector<HTMLTextAreaElement>(
            `[data-accessible-focused='true']`
          )
          ?.focus();
        ...
        if (a11yNote === undefined) {
          ...
          setA11yNote((prev) =>
            Math.min(notes.concat(notesToSend2Server).length - 1, prev! + 1)
          );
        }
      } else {
        return;
  ...
  const handleTrashButtonClick = () => {
    if (notesToSend2Server.find((n) => n.id === focusedNoteId)) {
      setNotesToSend2Server((prev) =>
        prev.filter((note) => note.id !== focusedNoteId)
      );
...
const InfoTableInnerWindow = ({ item }: { item: Transaction }) => {
  const { data: accountsData, isSuccess: accountsFetched } =
    useGetAccountsQuery();
    ...
    if (accountsFetched) {
      ...
      setInstitution(
        accountsData.institutions.find(
          (inst: any) => inst.id === account?.institution_id
        )
      );
      setAccount(account);
    }
  ...
  return (
    <NestedWindow2 className={styles.transactionDetails}>
      ...
      <div>Date </div>
      <div>{dayjs(item?.datetime || item?.date).format('MMM D, YYYY')}</div>
    ...
    <NestedWindow2 className={styles.billsAndCategories}>
      ...
      {!itemIsSplit ? (
        <div
          ...
          className={styles.billCatSelectorContainer}
        >
          <BillCatLabel
            as="button"
            ref={buttonContainerRef}
            ...
            slim={true}
            onClick={() => {
              setShowBillCatSelect(!showBillCatSelect);
            }}
          />
          <DropdownDiv
            placement="left"
...
const TransactionModal = withModal<{ item: Transaction; splitMode?: boolean }>(
  ({ item, splitMode }) => {
