...
export type BudgetItemSortOptions =
  | 'alpha-asc'
  | 'alpha-des'
  | 'limit-asc'
  | 'limit-des'
...
interface TSortContext {
  ...
  setCategoriesSort: React.Dispatch<
    React.SetStateAction<BudgetItemSortOptions | undefined>
  >;
  setBillsSort: React.Dispatch<
    React.SetStateAction<BudgetItemSortOptions | undefined>
  >;
}
...
export const SortProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <SortContext.Provider
      value={{ categoriesSort, billsSort, setCategoriesSort, setBillsSort }}
    >
      {children}
    </SortContext.Provider>
  );
