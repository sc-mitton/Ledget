...
import type { accountType } from '@features/transactionsSlice'
...
const Transactions = ({ cursor }: { cursor: string }) => {
    ...
    const { data: { results: transactionsData, next }, isSuccess } = useGetTransactionsQuery({
        ...
        type: typeMap[location.pathname.split('/')[3] as keyof typeof typeMap] as accountType,
        cursor: cursor
    })
...
const Deposits = () => {
    ...
    const { isSuccess: accountsAreFetched } = useGetAccountsQuery()
    const [syncTransactions, { isSuccess, isError, isLoading: isSyncing, data: syncResult }] =
    ...
    const handleScroll = (e: React.UIEvent<HTMLElement>) => {
        ...
        if (bottom) {
            setPaginationCursors((prev) => [...prev, searchParams.get('cursor') as string])
        }
    }
    ...
    return (
        <>
            ...
            <TransactionsTable
                ...
                shimmering={!searchParams.has('cursor')}
            >
                <TransactionsHeader />
                {accountsAreFetched &&
                ...
                <TransactionShimmer shimmering={paginationCursors.length > 1} />
            </TransactionsTable>
            <div className='refresh-btn--container' >
                <RefreshButton
                    loading={isSyncing}
                    onClick={() => {
                        syncTransactions(searchParams.get('account') as accountType)
                    }}
                />
            </div>
            <Outlet />
