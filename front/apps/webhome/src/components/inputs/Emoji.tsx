...
export interface EmojiProps
  extends Partial<Pick<EmojiContextType, 'emoji' | 'setEmoji'>> {
...
const Emoji = (props: EmojiProps) => {
  ...
  return (
    <EmojiContext.Provider value={{ emoji, setEmoji, picker, setPicker }}>
      <div className={styles.ledgetEmojiPicker}>
...
const EmojiPicker = () => {
  const { setEmoji, picker, setPicker } = useContext(
    EmojiContext
  ) as EmojiContextType;
...
const EmojiButton = ({ ...rest }) => {
  const { emoji, picker, setPicker } = useContext(
    EmojiContext
  ) as EmojiContextType;
