...
const PassWord = ({ onCancel }: { onCancel: () => void }) => {
  ...
  useEffect(() => {
    if (!sessionIsFreshAal1) fetchFlow({ aal: 'aal1', refresh: true });
  }, []);

  useEffect(() => {
    if (isCompleteSuccess) dispatch({ type: aal1ReAuthed });
  }, [isCompleteSuccess]);

  return (
    <form onSubmit={submit} className={styles.reauthForm}>
      <div>
        {['setup', 'change', 'delete'].some((word) =>
  ...
  return (
    <AnimatePresence mode="wait">
      {(!searchParams.get('aal') || searchParams.get('aal') === 'aal1') && (
        <SlideMotionDiv key="aal1" position={loaded ? 'first' : 'fixed'}>
          <PassWord onCancel={() => props.closeModal()} />
        </SlideMotionDiv>
      )}
      {searchParams.get('aal') === 'aal2' && (
        <SlideMotionDiv key="aal2" position={'last'}>
          <Totp />
        </SlideMotionDiv>
      )}
    </AnimatePresence>
...
const useReauthCheck = ({
  ...
  useEffect(() => {
    ...
    const aalGood =
      user?.highest_aal === 'aal1' || requiredAal === 'aal1'
        ? Boolean(reAuthed.level)
        : user?.highest_aal === reAuthed.level;
    ...
    return (
      <>
        <ReAuthModal
          ...
          zIndex={300}
        />
        <Component {...props} hideModal={!continueToComponent} zIndex={200} />
      </>
    );
  };
}
