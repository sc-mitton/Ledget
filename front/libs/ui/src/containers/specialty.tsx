...
export const ExpandableContainer = styled.div<{ expanded?: boolean, maxHeight?: number }>`
  opacity: ${({ expanded }) => expanded ? 1 : 0};
  max-height: ${({ expanded, maxHeight = 60 }) => expanded ? `${maxHeight}em` : 0};
...
export const GrowOnDiv = styled.div<{ immediate?: boolean }>`
  ...
  transform: scale(.95);
  opacity: 0;
  animation: ${({ immediate }) => immediate ? 'none' : growOn} 0.5s ease-in-out;
`
