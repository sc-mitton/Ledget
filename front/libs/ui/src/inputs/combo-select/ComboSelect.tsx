...
type ComboSelectProps<V extends OptionValue, O extends Option<V>> = (
  | ComboSelectPropsMultiple<V>
  | ComboSelectPropsSingle<V>
) & {
  limit?: number;
  name?: string;
  children?:
...
type TDataContextBaseProps = {
  ...
  setActive: React.Dispatch<
    React.SetStateAction<Option<OptionValue> | undefined>
  >;
...
type TDataContext = TDataContextBaseProps &
  ComboSelectProps<OptionValue, Option<OptionValue>>;
...
const useDataContext = () => {
  ...
  if (!context) {
    throw new Error(
      `ComboSelect compound components cannot be rendered outside the ComboSelect component`
    );
  }
  return context;
};
...
const ComboSelect = <
  TV extends OptionValue,
  TO extends { id: string | number; value: TV; disabled?: boolean }
>(
  ...
  return (
    <Component
      ref={(e) => {
        ...
        if (buttonRef) (buttonRef.current as any) = e;
      }}
      onClick={handleClick}
...
const Options = (props: OptionsProps) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLUListElement>) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        (
          ulRef?.current?.querySelector(`[headlessui-state='active']`) as any
        )?.click();
        break;
      case 'Tab':
      case 'Escape':
...
const Option = ({ option, disabled, children }: OptionProps) => {
  ...
  const updateOptions = () => {
    if (custom.includes(option)) {
      ...
      setCustom((prev) => prev.filter((item) => item !== option.value));
    }
  };
