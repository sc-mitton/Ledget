...
import Animated, { useSharedValue, withTiming, withRepeat, withDelay, withSequence, useAnimatedStyle } from 'react-native-reanimated';
...
export const LoadingDots = ({ visible }: { visible: boolean }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      opacity0.value = withDelay(0, withRepeat(withSequence(withTiming(1, config), withTiming(0, config),), -1,))
      opacity1.value = withDelay(150, withRepeat(withSequence(withTiming(1, config), withTiming(0, config),), -1,))
      opacity2.value = withDelay(350, withRepeat(withSequence(withTiming(1, config), withTiming(0, config),), -1,))
    } else {
